...
contract Mani is Context, IBEP20, Ownable {
	...
	*/
	function updateTaxExclusionAdd(address _addTaxExclusion) external onlyOwner {
	...
	* Can only be called by the current operator
	*/
	function updateTaxExclusionRemove(address _removeTaxExclusion) external onlyOwner {
    ...
    function setCanBurn(bool bCanBurn) onlyOwner external{
        _bBurn = bCanBurn;
    }
    function canPause() external view returns(bool){
        return _bPause;
    }
    function setCanPause(bool bCanPause) onlyOwner external{
        _bPause = bCanPause;
    }
